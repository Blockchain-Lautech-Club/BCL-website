...
export default function AboutSection() {
  return (
    <section className="relative overflow-hidden bg-white py-16 sm:py-20">
      ...
      <div className="absolute left-0 top-1/2 -translate-y-1/2 space-y-4 md:space-y-6 hidden md:block">
        {[...Array(7)].map((_, i) => (
          <div key={`left-${i}`} className="h-1 w-20 bg-gray-200 sm:w-40 md:h-2 md:w-80" />
        ))}
      </div>
      <div className="absolute right-0 top-1/2 -translate-y-1/2 space-y-4 md:space-y-6 hidden md:block">
        {[...Array(7)].map((_, i) => (
      ...
      {/* Center dotted line */}
      <div className="absolute left-1/2 top-0 h-full w-px border-l border-dashed border-gray-300 hidden md:block" />
    </section>
  );
}
